...
public class StudentFeeQueryServiceImpl implements StudentFeeQueryService {
     ...
     *
     *
     * @return {@link QrTokenResponseDTO}
     * @throws MemberException 회원이 존재하지 않는 경우
     */
    @Override
    public QrTokenResponseDTO generateStudentFeeQrToken() {
        ...
        boolean isStudentFeePaid = studentFeeStatus.equals(StudentFeeStatus.PAID);

        // qrToken 생성
        String qrToken = qrManager.generateQrToken(QrType.STUDENT_FEE, member.getId());

        // 응답 반환
        return QrConverter.toQrTokenResponseDTO(qrToken, isStudentFeePaid);
    }
